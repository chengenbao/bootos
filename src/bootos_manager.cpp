...
{
	Json::Value v;
	string command = "  cat /proc/cpuinfo | grep \"physical id\" | sort | uniq | wc -l ";
	string tmp;

	execute_command(command, tmp);
	int cpu_num = atoi(tmp.c_str());

	command = " cat /proc/cpuinfo | grep \"cpu cores\" | uniq | head -1 | awk -F\":\" '{print $2}' ";
	execute_command(command, tmp);
	int cores_per_cpu = 1;
...
{
	string command = " dmidecode -t 1 | grep \"Serial Number\" | awk -F: '{print $2}' ";
	string sn;
...
set<string> bootos_manager::BLOCK_COMMAND = bootos_manager::init_block_command();
map<string, task_func> bootos_manager::INNER_COMMAND = bootos_manager::init_inner_command();
map<string, string> bootos_manager::REGISTE_STATUS = bootos_manager::init_registe_status();
map<string, string> bootos_manager::IMM_CONFIG_STATUS = bootos_manager::init_imm_config_status();
