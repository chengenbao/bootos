...
void process_cmdline_file() {
  ...
  for (vector<string>::iterator it = lines.begin(); it != lines.end(); ++it) {
    ...
    for (vector<string>::iterator it1 = fields.begin(); it1 != fields.end();
         ++it1) {
...
bool split_string(const string &str,
  ...
  for (set<string>::iterator it = patterns.begin(); it != patterns.end();
       ++it) {
    if (*it == " ")
      continue;
...
string
replace_all(const string &src, const string symbol, const string target) {
  string result(src);
  int pos;
